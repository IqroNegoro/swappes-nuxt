<template>
  <div class="fixed top-0 left-0 w-full h-full bg-black/50 z-30 flex justify-center items-center"
    ...
    <Transition name="fade-up" appear mode="in-out">
      <form @submit.prevent="handleSubmitForm" ref="container"
        class="bg-primary text-white relative md:rounded-md w-full md:w-3/4 lg:w-1/2 md:max-h-[85%] max-h-full overflow-hidden flex flex-col"
        @click.stop>
