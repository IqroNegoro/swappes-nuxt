...
export default defineEventHandler({
    ...
    handler: async e => {
        ...
        try {
            ...
            const updated = await user.updateOne({ avatar: image });

            if (image) {
                if (user!.avatar && existsSync(join(process.cwd(), "public", "images", user!.avatar))) {
                    unlinkSync(join(process.cwd(), "public", "images", user!.avatar));
                }
            }
